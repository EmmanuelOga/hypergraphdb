...
{
	public static final String SCHEME_NAME = "javaclass";
...
//  private TxCacheMap<Class<?>, HGHandle> classToAtomType = null; //new ClassToTypeCache();
	...
	public boolean handles(String scheme) {
		return SCHEME_NAME.equals(scheme);
	}

	public boolean isPresent(HyperGraph graph)
	{
		return graph.getTypeSystem().getHandleForIdentifier(toTypeURI(HGPlainLink.class)) != null;
	}

	public void initialize(HyperGraph graph, HGTypeConfiguration typeConfiguration)
	{
		this.graph = graph;

		graph.getTypeSystem().uriToAtomType.load(toTypeURI(Top.class), graph.getHandleFactory().topTypeHandle()); // TOP is its own type
		graph.getTypeSystem().uriToAtomType.load(toTypeURI(Object.class), graph.getHandleFactory().topTypeHandle()); // TOP also corresponds to the java.lang.Object "top type"
		graph.getTypeSystem().uriToAtomType.load(toTypeURI(HGLink.class), graph.getHandleFactory().linkTypeHandle());
		graph.getTypeSystem().uriToAtomType.load(toTypeURI(HGSubsumes.class), graph.getHandleFactory().subsumesTypeHandle());
		...
		if (!isPresent(graph))
		{
			...
			for (HGPersistentHandle typeHandle : config.getHandles())
			{
				...
				if (targets.isEmpty())
					...
					graph.getTypeSystem().addPredefinedType(typeHandle, typeInstance, toTypeURI(target));
			}
		}
	...
	public HGAtomType toRuntimeType(HGHandle typeHandle, HGAtomType typeInstance, URI uri)
	{
		HGAtomType result = typeInstance;
		String classname = uriToClassName(uri);
		if (classname != null)
		{
			Class<?> clazz;
			try
			...
			graph.getTypeSystem().uriToAtomType.load(uri, typeHandle);
		}
		return result;
	}
	...
	public static URI classNameToURI(String classname)
	{
		try
		{
			return new URI(SCHEME_NAME + ':' + classname);
		}
	...
	{
		Class<?> clazz = getTypeDescriptor(typeId);
		Map<URI, HGHandle> m = graph.getTypeSystem().getLocalIdMap();
		m.put(typeId, typeHandle);
