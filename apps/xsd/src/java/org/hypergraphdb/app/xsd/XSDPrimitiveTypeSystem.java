...
/**
 * Registers the XSD primitive types. Uses an XMLSchema.xsd to import them as HGDB types. It also
 * statically defines some rules and relationships regarding the types.
 */
public class XSDPrimitiveTypeSystem extends DefaultHandler
{
    ...
    private final Map<String,
                      Class> primitiveNames = new HashMap<String, Class>();
    private final Map<String,
                      Class> constrainingFacets = new HashMap<String, Class>();
    private final Map<String,
        Object> fundamentalFacets = new HashMap<String, Object>();
     ...
     * @param hg HyperGraph
     */
    public void bootstrap(final HyperGraph hg)
    {
        ...
        HGSearchResult<HGHandle>
            rs = hg.find(new AtomTypeCondition(HGListenerAtom.class));
        while (rs.hasNext())
        {
            HGHandle lsnrHandle = rs.next();
            HGListenerAtom lsnr = (HGListenerAtom) hg.get(lsnrHandle);
            if (lsnr.getEventClassName().equals(regListener.
                getEventClassName())
                &&
                lsnr.getListenerClassName().equals(regListener.
    ...
    /**
     * Define the primitive types using the specified XMLSchema.xsd file as a source.
     * @param xmlschemaFile an XMLSchema.xsd file
     */
    public void importPrimitives(final HyperGraph hg, String xmlschemaFile)
    {
        this.hg = hg;
                             ...
                             Attributes attributes)
    {
        ...
        {
            handleHasProperty(attributes.getValue("name"),
     ...
     */
    private void handleSimpleType(String primitiveName)
    {
        ...
        if (null != handler)
        {
            ...
            /**@todo Set up the "namespace" link; in the real code that link will be
             * queried for and then the new atom type will be added to its target
             * list.*/
...
//        HGHandle namespaceHandle = hg.add(namespace);
     ...
     *
     */
    private void handleHasFacet(String facetName)
    {
     ...
     *
     */
    private void handleHasProperty(String name, String value)
    {
    ...
    /**
     * Completes the registration of a primitive type and resets the members that are involved in
     * the process.
     */
